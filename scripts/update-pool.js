...
function buildTeamStats(matches) {
  ...
  for (const m of matches) {
    ...
    const homeGoals = m.score.fullTime.homeTeam;
    const awayGoals = m.score.fullTime.awayTeam;
...
// group membership. football-data.org returns `group` on group-stage
// matches (e.g. "Group A"). We rank teams within each group by points,
// then goal difference, then goals scored — standard tiebreakers — and
// mark the bottom two in each group of four as eliminated UNLESS the
// group stage isn't finished yet (some teams still have games left).
